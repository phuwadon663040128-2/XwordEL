...
'''
# This code is a modified version of the original crossword.py
# The original code is available at https://github.com/dnoegel/crossword-generator/

# Modified by: Tanawat Sombatkamrai (XwordEL project)

# The modifications are:
- change the code to be able to run on python 3 and suitable for XwordEL project
- remove a few unnecessary code for the purpose of XwordEL project
*** please note that this code is not a complete code ,I just modify the code to be able to run in some specific case
*** if you want to use this code for other purpose, you may need to modify the code by yourself
...
class MaxLoopError(Exception):
    def __init__(self, message, count, word=None):
...
def stats(cwd, print_missing=True):
    ...
    print(("%f crosses per word" %
          (float(score-len(cwd.placed_words))/len(cwd.placed_words))))
    ...
    print(("total-cells/empty-cells- Quotient: %.1f/1 (%i, %i)" %
          (round(num_cells/num_empty, 1), num_cells, num_empty)))
...
def run_benchmark_test(words=100, num=100, bestof=3):
    ...
    cwds = multiple_crosswords(
        "auto", "auto", " ", 5000, word_list, num=num, best_of=bestof, force_solved=False)
    ...
    # , solution="I solved it")
    formatter = CrossWordFormatter(best_crossword, ppb=32)
    ...
    formatter.get_crossword_image_grid(
        output="benchmark-output-file.png", solved=True)
    ...
    def parse(self, filename):
        ...
        for line in content:
            ...
            if rg:
                ...
                # Match key = value
                rg = re.match(
                    r"\s*(?P<key>.+?)[ \t]*?[:=][ \t]*(?P<value>.+)", line)
                if rg:
                    if rg.group("key").strip().lower() == "question first":
                        self.dict[section]["question first"] = rg.group(
                            "value").lower().strip().startswith("t")
                    else:
                        self.dict[section][rg.group("key").strip(
                        ).lower()] = rg.group("value").strip()

        if not "options" in self.dict:
            self.dict["options"] = {}

        if not "question first" in self.dict["options"]:
            self.dict["options"]["question first"] = True
            ...
            # ~ elif section == None:
                ...
                if rg:
                    if self.dict["options"]["question first"]:
                        self.dict[section].append(
                            (rg.group("value").strip(), rg.group("key").strip()))
                    else:
                        self.dict[section].append(
                            (rg.group("key").strip(), rg.group("value").strip()))
...
class CrossWordFormatter(object):
    ...
    def __init__(self, crossword, ppb=32, solution=None, transparency=False, order=True):
        ...
        self.highlight_colors = [(80, 73, 158, 128), (75, 191, 84, 128), (
            219, 61, 101, 128), (237, 230, 36, 128), (132, 215, 240, 128)]
    ...
    def _next_highlight_color(self):
        ...
        try:
            ...
            print(
                "Generating random highlight color - please add additional highlight colors")
            self.colors["highlight"] = (random.randint(
                0, 255), random.randint(0, 255), random.randint(0, 255), 128)
    ...
    def _set_solution(self, solution):
        ...
        for letter in solution:
            ...
            if not letter in self.crossword.letters or self.crossword.letters[letter] == []:
                raise SolutionError(
                    "Cannot mark solution-letter '%s': No '%s' in this crossword" % (letter, letter))
            if solution.count(letter) > len(self.crossword.letters[letter]):
                raise SolutionError("Your solution '%(solution)s' has %(num)i '%(letter)s' - there are not that much '%(letter)s's in your crosssword!" % {
                                    "num": solution.count(letter), "letter": letter, "solution": solution})
        ...
        for r in range(self.crossword.rows):
            ...
            for c in self.crossword.grid[r]:
                if c == self.crossword.empty:
                    outStr += '%s%s' % (string.lowercase[random.randint(
                        0, len(string.lowercase)-1)], printstr)
                else:
    ...
    def get_crossword_ascii_grid(self, solved, printable=True):
        ...
        outStr = ""
        """
        for w in self.crossword.placed_words:
            # print(w.word, w.col, w.row, w.vertical, w.number)
            print(f"word = {w.word}, col = {w.col}, row = {
                  w.row}, number = {w.number},vertical = {not w.vertical}")
        """
        ...
        #
        # print(len(self.crossword.grid), len(self.crossword.grid[0]))
        # print(self.crossword.grid)
        # print("----------------------------------")
        # print(self.crossword.rows)
        # for r in range(len(self.crossword.grid)):
        '''
        for r in range(self.crossword.rows):
            print(f"row = {r}")
            ...
            outStr += '\n'
        '''
        for rows in range(self.crossword.rows):
            for cols in range(self.crossword.cols):
                outStr += f'{self.crossword.grid[cols][rows]}{printstr}'
            outStr += '\n'
    ...
    def _draw_arrow(self, draw, dir, x, y, w, h):
        if dir == "right":
            ...
            raise Exception(
                "What the... ? I just can draw right and down arrows!")
...
def wordlist_from_string(str, get_crossword_solution_delimiter="/", question_delimiter="\n", get_crossword_solution_first=True):
    ...
    for line in lines:
        if get_crossword_solution_first:
            get_crossword_solution, question = line.split(
                get_crossword_solution_delimiter)
        else:
            question, get_crossword_solution = line.split(
                get_crossword_solution_delimiter)
...
def multiple_crosswords(cols, rows, empty="-", maxloops=2000, wordlist=[], num=10, time_permitted=2.0, best_of=10, force_solved=False):
    ...
    for i in range(0, num):
        ...
        score = cross.compute_crossword(
            best_of=best_of, force_solved=force_solved)
...
class CrossWord(object):
    ...
    def __init__(self, cols, rows, empty='_', maxloops=2000, wordlist=[], reduce=None):
        ...
        if len(wordlist) < 3:
            ...
            raise WordListError(f"Need at least 3 entries!, got {wordlist}({len(wordlist)})")
        '''
        ...
        if cols == "auto" or rows == "auto":
            ...
            # ~ if not reduce:
            logging.debug(
                "'%s': %i - Average: %i" % (longest, min_length, average))
    ...
    def _setup_grid_and_letters(self):
        ...
        self.letters = {}
        for letter in str(string).lower():
            self.letters[letter] = []
    ...
    def compute_crossword(self, rounds=2, best_of=3, force_solved=False):
        ...
        copy = CrossWord(self.cols, self.rows, self.empty, self.maxloops, [
                         (w.word, w.clue) for w in self.wordlist], reduce=reduce)
        ...
        while (count <= best_of-1 and not force_solved) or (force_solved and not solved):
            ...
            if force_solved and count >= self.maxloops:
                raise MaxLoopError(
                    "Could not solve the crossword within %i tries" % self.maxloops)
    ...
    def _place_word(self, word):
        ...
        if count >= self.maxloops:
            raise MaxLoopError(
                "Maxloops reached - canceling (Counter: {%i}, Word: {%s})" % (count, word.word), count=count, word=word.word)
...
if __name__ == "__main__":
    ...
    input_file_path = r'D:\0\experiment\testttt\crosswords\lot_questions.cwf'
    parser = SimpleParser(input_file_path)
